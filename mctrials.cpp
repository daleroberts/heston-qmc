...
int main(int argc, char *argv[]) 
{
    ...
    for (int powerTwo = 3; powerTwo <= maxPowerTwo; ++powerTwo)
    {
        int numberPaths = 1 << 14; //14=16384
        int timesteps = 1 << powerTwo;
        
        printf("%6i", timesteps);
        for (int j = 6; j <= 6; ++j) {
            Result res = run(r, rho, kappa, theta, sigma, v0, S0, T, K, j, numberPaths, timesteps, seed);
            printf(" & %.5f (%.5f) & %.5f", res.mean, res.stderr, abs(res.mean - 6.8061));
            if (j == 6) printf(" \\\\\n");
        }
